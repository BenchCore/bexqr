var bexqr_selector = "bex-qr",
    bexqr_min_height = 400,
    bexqr_min_width = 400,
    bexqr_auto = true,
    bexqr_padding = [10, 10];
...
(function() {
  $(document).ready(function() {
    ...
    $(window).resize(function() {
      if(bexqr_auto) {
        $(bexqr_selector).each(function(i, e) {
          image_fit(e, false);
        });
      }
    });
  ...
  function image_toggle(image, animate) {
    $("<bex-qr/>") // Make in memory copy of image to avoid css issues
  ...
  function image_fit(image, animate) {
    ...
    .load(function() {
      var actual_width = this.width,   // Note: $(this).width() will not
          ...
          target_width = $(window).width() - bexqr_padding[0],
          target_height = $(window).height() - bexqr_padding[1],
          new_height = 0,
          new_width = 0,
          w_width, w_height, h_width, h_height;

      // do not bother with images that are already smaller than the minima
      if(actual_height < bexqr_min_height) { return; }
      if(actual_width < bexqr_min_width) { return; }
      ...
      if(aspect_ratio > 2) {
        ...
        if(actual_width < actual_height) {
          new_width = target_width > bexqr_min_width ? target_width : bexqr_min_width;
          new_height = compute_height(new_width);
        } else {
          new_height = target_height > bexqr_min_height ? target_height : bexqr_min_height;
          new_width = compute_width(new_height);
        }
      } else {
        // fit image entirely within viewport
        w_width = target_width > bexqr_min_width ? target_width : bexqr_min_width;
        w_height = compute_height(w_width);
        h_height = target_height > bexqr_min_height ? target_height : bexqr_min_height;
